...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const getIndexByWord = (word, text, caseSensitive) => {
    if (true) { // CONFIG.localsearch.unescape = true
  ...
  const inputEventFunction = () => {
    ...
    if (keywords.length === 1 && keywords[0] === '') {
      resultContent.innerHTML = '<div id="no-result"><i class="fas fa-search fa-5x fa-fw"></i></div>';
    } else if (resultItems.length === 0) {
      resultContent.innerHTML = '<div id="no-result"><i class="far fa-frown fa-5x fa-fw"></i></div>';
    } else {
  ...
  const fetchData = () => {
    fetch('' + searchPath) // CONFIG.root = ''
      ...
      .then(res => {
        ...
        document.getElementById('no-result').innerHTML = '<i class="fas fa-search fa-5x fa-fw"></i>';
        inputEventFunction();
      });
  };
